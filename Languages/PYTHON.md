...
## <a name="sokets"></a>Сокеты
+ Сокеты встроены в Python в виде стандартной библиотеки **`socket`**. Первичные функции и методы этого модуля следующие:
  + socket() - создать сокет;
  + bind() - привязывает сокет к конкретному хосту и порту, зависит от аресного семейства;
  + listen() - создает слушающий сокет;
  + accept() - блокирует и ждет подключения клиента. Когда клиент подключается, он возвращает новый объект сокета, представляющий соединение, и кортеж, содержащий адрес клиента. Кортеж содержит host и port;
  + connect();
  + connect_ex();
  + send() - шлет данные в сокет;
  + recv() - читает данные из сокета;
  + close() - закрывает сокет;
+ Создать сокет можно следующим образом:
  ```python
  with socket() as s:
      print("HERE")
  ```
  + При этом, при его создании можно передать различные параметры, которые конфигурируют соединение. Они представлены как константы в модуле socket:
    + Следующие константы определяют адресное семейство. Являются первым элементом в создании сокета;
      + socket.AF_UNIX - адрес привязан к узлу файловой системы, представлен в виде строки с использованием кодировки файловой системы. Адрес является байтовым объектом в пространстве имен Линукса. Строковый или байтовый объект может использоваться для любого типа адреса при передаче его в качестве аргумента.
      + socket.AF_INET - пара host, port, где host - это либо строка либо IPv4 одрес, а port - это integer.
      + socket.AF_INET6 - четыре элемента host, port, flowinfo, scope_id, где flowinfo и scope_id представляют члены sin6_flowinfo и sin6_scope_id в struct sockaddr_in6 в C.
    + Константы для типа протокола, используютя 2м аргументом при инициализации сокета:
