...
+ Для того чтобы создать массив в бд, необходимо указать об этом еще на стадии создания таблицы:
    ```
    create table posts(
        id serial primary key,
        title varchar(30),
    ...
    ```
+ Для того чтобы поместить новый объект с массивом в бд, необходимо взять массив в одинарные ковычки и фигурные скобки, а каждый объект внутри массива(если он строка) взять в двойные ковычки:
    ```
...
+ **`CREATE TYPE ... AS ENUM (...);`** - создание перечисления:
    ...
    ```
    Добавление такое же как и в обычные таблицы
    ```
...
### <a name="intersect"></a> Пересечение множеств
+ **`INTERSECT ... ;`** - позволяет найти общее для 2х множеств.
    ```
    SELECT FirstName, LastName
    FROM Employees
    INTERSECT 
