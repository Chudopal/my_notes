...
+ **DNS-сообщение** представляет из себя 12-байтовый заголовок фиксированной длины и переменное тело, в котором передается запросы и ответы с сервера.
    + Запрос DNS:
        ...
        + Записи ответов - ресурсные записи с ответом на вопросы
    ...
    + **Записи запросов** состоит из имени вопроса и типа вопроса:
        + Имя запроса - запрашиваемое доменное имя
        + Тип запроса - 16-разрядное целое число, используемое для представления типа записи ресурса, самые популярные из них:
            + 0x01 - Запись хоста (A)
